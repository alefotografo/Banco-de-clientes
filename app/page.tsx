...
export default function Home() {
  ...
  async function saveSearch() {
    if (!supabase || !session) return setNotice("Entre para salvar esta busca.");
    if (!searchName.trim() || !query.city || !query.segment) return setNotice("Informe um nome e faça a busca antes de salvar.");
    const { error } = await supabase.from("prospex_searches").insert({ user_id: session.user.id, name: searchName.trim(), city: query.city, neighborhood: query.neighborhood || null, segment: query.segment, radius: Number(query.radius) });
    setNotice(error ? "Não foi possível salvar a busca." : `Busca “${searchName.trim()}” salva no seu espaço.`);
    if (!error) setSearchName("");
  }

  async function signIn(event: FormEvent) {
    event.preventDefault();
    if (!supabase) return setAuthNotice("O acesso seguro ainda não foi configurado.");
    const { error } = await supabase.auth.signInWithPassword({ email: email.trim(), password });
    setAuthNotice(error ? "E-mail ou senha inválidos." : "Acesso liberado.");
  }

  async function updatePassword(event: FormEvent) {
    event.preventDefault();
    if (!supabase || password.length < 8) return setAuthNotice("Use uma senha com pelo menos 8 caracteres.");
    const { error } = await supabase.auth.updateUser({ password });
    setAuthNotice(error ? "Não foi possível salvar a senha." : "Senha criada. Você já pode usar o Prospex.");
    if (!error) { setRecoveryMode(false); setPassword(""); }
  }

  async function requestPasswordReset(event: FormEvent) {
    event.preventDefault();
    if (!supabase || !email.trim()) return setAuthNotice("Informe seu e-mail corporativo.");
    const { error } = await supabase.auth.resetPasswordForEmail(email.trim(), { redirectTo: `${window.location.origin}/` });
    setAuthNotice(error ? "Não foi possível enviar a recuperação." : "Enviamos a recuperação. Verifique seu e-mail corporativo.");
  }
  ...
  return <main>
    ...
    {authOpen && !session && <div className="auth-overlay" role="dialog" aria-modal="true"><div className="auth-modal"><button className="auth-close" aria-label="Fechar" onClick={() => setAuthOpen(false)}>×</button><p className="eyebrow">ACESSO DO TIME</p><h2>{recoveryMode ? "Crie sua senha" : resetMode ? "Recuperar senha" : "Entrar no Prospex"}</h2><p>{recoveryMode ? "Defina uma senha com pelo menos 8 caracteres." : resetMode ? "Enviaremos um link para criar uma nova senha." : "Use seu e-mail corporativo e sua senha."}</p>{recoveryMode ? <form onSubmit={updatePassword}><input type="password" required minLength={8} placeholder="Nova senha" value={password} onChange={(event) => setPassword(event.target.value)}/><button className="primary full">Salvar nova senha</button></form> : resetMode ? <form onSubmit={requestPasswordReset}><input type="email" required placeholder="seu@email.com" value={email} onChange={(event) => setEmail(event.target.value)}/><button className="primary full">Enviar recuperação</button></form> : <form onSubmit={signIn}><input type="email" required placeholder="seu@email.com" value={email} onChange={(event) => setEmail(event.target.value)}/><input type="password" required minLength={8} placeholder="Sua senha" value={password} onChange={(event) => setPassword(event.target.value)}/><button className="primary full">Entrar</button></form>}<small>{authNotice}</small>{!recoveryMode && <button className="auth-link" onClick={() => { setResetMode(!resetMode); setAuthNotice(""); }}>{resetMode ? "Voltar ao login" : "Esqueci minha senha"}</button>}</div></div>}
